...
export default function MakeItYours(){
  const [step,setStep]=useState(0),[doorwayPicks,setDoorwayPicks]=useState([]),[neighborhoods,setNeighborhoods]=useState([]),[details,setDetails]=useState([]),[granular,setGranular]=useState([]),[extra,setExtra]=useState(""),[name,setName]=useState(""),[loaded,setLoaded]=useState(false);
  useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||"null");if(saved){setStep(Math.min(saved.step||0,4));setDoorwayPicks(saved.doorwayPicks||[]);setNeighborhoods(saved.neighborhoods||[]);setDetails(saved.details||[]);setGranular(saved.granular||[]);setExtra(saved.extra||"");setName(saved.name||"");}}catch{}setLoaded(true)},[]);
  useEffect(()=>{if(loaded)localStorage.setItem(STORAGE_KEY,JSON.stringify({step,doorwayPicks,neighborhoods,details,granular,extra,name,updatedAt:new Date().toISOString()}))},[step,doorwayPicks,neighborhoods,details,granular,extra,name,loaded]);
  ...
  const neighborhoodOptions=useMemo(()=>{const groups=chosenTopics.map(topic=>topic.children.map(label=>({label,parent:topic.label,color:topic.color,size:["Classical","Jazz","Rock","Cinema","Baseball","Architecture","Painting","Restaurants"].includes(label)?"lg":"md"})));return roundRobin(groups,96).filter((item,index,array)=>array.findIndex(other=>other.label===item.label)===index)},[broad]);
  const detailOptions=useMemo(()=>{const groups=neighborhoods.map((label,index)=>optionsFor(label).map(value=>({label:value,parent:label,color:topics[(index*3)%topics.length].color,size:index%4===0?"lg":index%5===0?"sm":"md"})));return roundRobin(groups,120).filter((item,index,array)=>array.findIndex(other=>other.label===item.label)===index)},[neighborhoods]);
  const granularOptions=useMemo(()=>{const groups=details.map((label,index)=>granularFor(label).map(value=>({label:value,parent:label,color:topics[(index*5+2)%topics.length].color,size:index%5===0?"lg":index%4===0?"sm":"md"})));return roundRobin(groups,120).filter((item,index,array)=>array.findIndex(other=>other.label===item.label)===index)},[details]);
  const profile=useMemo(()=>({version:3,name:name.trim(),title:name.trim()?`${name.trim()}’s Edition`:"My Edition",openingChoices:doorways.filter(item=>doorwayPicks.includes(item.id)).map(item=>item.label),broadInterests:chosenTopics.map(topic=>topic.label),specificInterests:neighborhoods,details,granularInterests:granular,anythingElse:extra.split(/,|\n/).map(value=>value.trim()).filter(Boolean),readerDefaults}),[name,doorwayPicks,chosenTopics,neighborhoods,details,granular,extra]);
  const progress=["Your passions","Genres + subjects","Go deeper","Fine-tune it","Ready"];
  const reset=()=>{if(confirm("Clear these choices and begin again?")){localStorage.removeItem(STORAGE_KEY);setStep(0);setDoorwayPicks([]);setNeighborhoods([]);setDetails([]);setGranular([]);setExtra("");setName("")}};
  ...
  const toggleDoorway=item=>{const nextPicks=toggle(doorwayPicks,item.id),nextSignals=new Set(doorways.filter(option=>nextPicks.includes(option.id)).flatMap(option=>option.signals)),allowedTopics=topics.filter(topic=>nextSignals.has(topic.id)),allowedNeighborhoods=new Set(allowedTopics.flatMap(topic=>topic.children)),keptNeighborhoods=neighborhoods.filter(value=>allowedNeighborhoods.has(value)),allowedDetails=new Set(keptNeighborhoods.flatMap(optionsFor)),keptDetails=details.filter(value=>allowedDetails.has(value)),allowedGranular=new Set(keptDetails.flatMap(granularFor));setDoorwayPicks(nextPicks);setNeighborhoods(keptNeighborhoods);setDetails(keptDetails);setGranular(granular.filter(value=>allowedGranular.has(value)))};
  const toggleNeighborhood=label=>{if(neighborhoods.includes(label)){const removed=new Set(optionsFor(label)),keptDetails=details.filter(value=>!removed.has(value)),allowedGranular=new Set(keptDetails.flatMap(granularFor));setNeighborhoods(neighborhoods.filter(value=>value!==label));setDetails(keptDetails);setGranular(granular.filter(value=>allowedGranular.has(value)))}else setNeighborhoods([...neighborhoods,label])};
  const toggleDetail=label=>{if(details.includes(label)){const removed=new Set(granularFor(label));setDetails(details.filter(value=>value!==label));setGranular(granular.filter(value=>!removed.has(value)))}else setDetails([...details,label])};
  return <main className="app interviewApp">
    ...
    {step===1&&<section className="screen"><StepHeader eyebrow="CHOOSE THE WIDE CANVAS" title="Which parts are yours?" copy="Every passion you picked opens here. Choose as many genres and subjects as you like."/><div className={`constellation neighborhoods ${neighborhoods.length?"hasSelection":""}`}>{neighborhoodOptions.map((item,index)=><Bubble key={`${item.parent}-${item.label}`} {...item} depth={1} index={index} selected={neighborhoods.includes(item.label)} onClick={()=>toggleNeighborhood(item.label)}/>)}</div></section>}
    {step===2&&<section className="screen"><StepHeader eyebrow="NOW GO DEEPER" title="What kind, exactly?" copy="These are subgenres of everything you just chose—not a random grab bag."/><div className={`constellation details ${details.length?"hasSelection":""}`}>{detailOptions.map((item,index)=><Bubble key={`${item.parent}-${item.label}`} {...item} depth={2} index={index} selected={details.includes(item.label)} onClick={()=>toggleDetail(item.label)}/>)}</div></section>}
    {step===3&&<section className="screen"><StepHeader eyebrow="ONE LAST PASS" title="Let’s get wonderfully specific." copy="Pick the artists, eras, leagues, crafts and deep cuts that feel especially you."/><div className={`constellation details ${granular.length?"hasSelection":""}`}>{granularOptions.map((item,index)=><Bubble key={`${item.parent}-${item.label}`} {...item} depth={3} index={index} selected={granular.includes(item.label)} onClick={()=>setGranular(toggle(granular,item.label))}/>)}</div><div className="optional"><label><span>Anything we missed? <i>Optional</i></span><input value={extra} onChange={event=>setExtra(event.target.value)} placeholder="Toss in an artist, director, team, author, style, place—anything."/></label></div></section>}
    {step===4&&<section className="screen finish"><StepHeader eyebrow="THAT’S PLENTY TO BEGIN" title="Your edition is ready." copy="Meanwhile can learn the rest while you enjoy it."/><div className="profile"><div className="profileName"><span>Name your edition <i>Optional</i></span><input value={name} onChange={event=>setName(event.target.value)} placeholder="Your first name"/><h2>{profile.title}</h2></div><div className="profileCloud">{[...profile.broadInterests,...neighborhoods,...details,...granular,...profile.anythingElse].slice(0,30).map((item,index)=><span className={`p-${index%5}`} key={`${item}-${index}`}>{item}</span>)}</div><div className="promise"><b>Already taken care of</b><p>The playful Reader design, mobile layout, ambient radio, rage-free editorial rules, source variety, duplicate protection and seven-day memory are all built in. You can teach it more with <em>More like this</em> and <em>Less</em> while you browse.</p></div></div></section>}
    <nav><button disabled={step===0} onClick={()=>setStep(value=>Math.max(0,value-1))}>← Back</button>{step<3&&<button className="primary" disabled={step===0?!doorwayPicks.length:step===1?!neighborhoods.length:step===2?!details.length:false} onClick={next}>{step===0?"Choose my genres":step===1?"Go deeper":"Fine-tune it"}<span>→</span></button>}{step===3&&<button className="primary" onClick={next}>This feels like me <span>→</span></button>}{step===4&&<button className="primary" onClick={buildEdition}>Open my edition <span>→</span></button>}</nav>
