...
const prioritizeUnseen = items => {
  const seen = new Set(JSON.parse(localStorage.getItem("betterStartGenericSeen") || "[]"));
...
export default function Home() {
  ...
  useEffect(() => {
    setSaved(JSON.parse(localStorage.getItem("betterStartGenericSaved") || "[]")); setJoyHistory(recentHistory("betterStartGenericJoyHistory"));
    let lastLoad = Date.now();
    const loadEdition = async preserve => {
      const visit = `${Math.floor(Date.now() / EDITION_MS)}-${Date.now()}-${Math.random()}`, mediaHistory = recentHistory("betterStartGenericMediaHistory"), avoid = [...new Set(mediaHistory.map(entry => entry.id))].slice(-120).join(",");
      try { const next = await (await fetch(`/api/feed?visit=${encodeURIComponent(visit)}&avoid=${encodeURIComponent(avoid)}`, {cache: "no-store"})).json(); setJoyHistory(recentHistory("betterStartGenericJoyHistory")); setData(previous => prepareEdition(next, previous, preserve)); setEditionNote(`${preserve ? "Freshened" : "New"} ${new Date().toLocaleTimeString([], {hour: "numeric", minute: "2-digit"})} edition`); lastLoad = Date.now(); } catch {}
    };
    loadEdition(false);
  ...
  useEffect(() => { if (!wall.length) return; const visible = wall.slice(0, batches * BATCH_SIZE), seen = JSON.parse(localStorage.getItem("betterStartGenericSeen") || "[]"), combined = [...new Set([...seen, ...visible.map(itemKey)])].slice(-800), nowSeen = Date.now(); localStorage.setItem("betterStartGenericSeen", JSON.stringify(combined)); const media = recentHistory("betterStartGenericMediaHistory"), mediaIds = new Set(media.map(entry => entry.id)); visible.filter(item => item.videoId && !mediaIds.has(item.videoId)).forEach(item => media.push({id: item.videoId, ts: nowSeen})); localStorage.setItem("betterStartGenericMediaHistory", JSON.stringify(media.slice(-300))); const joy = recentHistory("betterStartGenericJoyHistory"), joyIds = new Set(joy.map(entry => entry.signature)); visible.filter(item => item.signature && !joyIds.has(item.signature)).forEach(item => joy.push({signature: item.signature, ts: nowSeen})); localStorage.setItem("betterStartGenericJoyHistory", JSON.stringify(joy.slice(-300))); }, [wall, batches]);
  const rate = (item, action) => { const ratings = JSON.parse(localStorage.getItem("betterStartGenericFeedback") || "[]"); ratings.push({url: item.url, title: item.title, source: item.source, action, ts: Date.now()}); localStorage.setItem("betterStartGenericFeedback", JSON.stringify(ratings.slice(-250))); };
  const toggleSave = item => setSaved(current => { const exists = current.some(savedItem => itemKey(savedItem) === itemKey(item)), next = exists ? current.filter(savedItem => itemKey(savedItem) !== itemKey(item)) : [{...item, savedAt: Date.now()}, ...current]; localStorage.setItem("betterStartGenericSaved", JSON.stringify(next.slice(0, 200))); return next.slice(0, 200); });
  ...
  return <main className="shell">
    <header className="mast"><div><div className="brand">Better Start <i>— Good Things Edition</i></div><div className="edition">Rage-free news, discovery & good times</div></div><div className="mastTools"><button className="savedButton" onClick={() => setShowSaved(value => !value)}>Saved <b>{saved.length}</b></button><button className={`radio ${radio ? "radioOn" : ""}`} onClick={() => setRadio(!radio)} aria-label={`Better Start Radio ${radio ? "on" : "off"}`} title="Better Start Radio placeholder"><span>♪</span><small>{radio ? "ON" : "RADIO"}</small></button></div></header>
    ...
    <section className="favoritesSection"><div className="sectionHead"><div><span>A few especially nice things</span><h2>Bright Spots</h2></div><p>Kindness, ingenuity & excellent dogs</p></div><div className="favorites">{(data?.favorites || []).map(item => <a className="favorite" href={item.url} target="_blank" rel="noreferrer" key={item.canonicalUrl}><span>{age(item.date)}</span><h3>{item.title}</h3><b>{item.source}</b></a>)}</div></section>
