...
const absoluteSafetyBlocked = value => emergencyBlocked.test(value) || civicBlocked.test(value) || barredTechEmpireBlocked.test(value);
...
const strictStoryUrl = value => canonicalStoryUrl(value).split("?")[0].replace(/\/(?:amp|index\.html?)$/i, "").replace(/\/$/, "");
const strictStoryTitle = value => {
  const withoutPublisher = String(value || "").split(/\s+(?:\||—|–|-)\s+(?=[^|—–-]{2,45}$)/)[0];
  return normalizedIdentityTitle(withoutPublisher).replace(/\b(?:npr|jstor daily|colossal|the guardian|guardian|associated press|ap news|new york times|washington post)\b$/i, "").trim();
};
...
const claimUnique = (items = [], seen = new Set()) => items.filter(item => {
  ...
  if (bannedSource(item) || routineSportsBlocked.test(safetyText) || absoluteSafetyBlocked(safetyText) || religionBlocked.test(safetyText) || educationCultureWarBlocked.test(safetyText) || corporateAmazonBlocked(safetyText) || retiredRepeat.test(safetyText)) return false;
...
const claimSessionUnique = (items = [], reserved = []) => {
  ...
  return (items || []).filter(item => {
    const safetyText = `${item?.title || ""} ${item?.summary || ""} ${item?.source || ""} ${item?.section || ""} ${item?.url || ""}`;
    const keys = identityKeys(item);
    if (!keys.length || bannedSource(item) || routineSportsBlocked.test(safetyText) || absoluteSafetyBlocked(safetyText) || religionBlocked.test(safetyText) || educationCultureWarBlocked.test(safetyText) || corporateAmazonBlocked(safetyText) || retiredRepeat.test(safetyText)) return false;
...
function AccountPanel({user, email, setEmail, password, setPassword, mode, setMode, status, onSubmit, onSignOut, onClose}) {
  const copy = mode === "signup" ? ["Create your account.", "Use your email and choose a password.", "Create account"] : mode === "forgot" ? ["Reset your password.", "We’ll email you one secure reset link.", "Send reset link"] : mode === "update" ? ["Choose a new password.", "Use at least eight characters.", "Save new password"] : ["Welcome back.", "Sign in with your email and password.", "Sign in"];
  return <div className="accountVeil" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="accountPanel" role="dialog" aria-modal="true" aria-labelledby="account-title"><button className="accountClose" onClick={onClose} aria-label="Close account window">×</button><span className="accountEyebrow">Your Meanwhile</span><h2 id="account-title">{user && mode !== "update" ? "You’re signed in." : copy[0]}</h2>{user && mode !== "update" ? <><p>Your preferences, saved stories and reading history can now follow you between devices.</p><strong className="accountAddress">{user.email}</strong><div className="accountActions"><button onClick={onClose}>Keep reading</button><button className="accountSecondary" onClick={onSignOut}>Sign out</button></div></> : <><p>{copy[1]}</p><form onSubmit={onSubmit}>{mode !== "update" && <><label htmlFor="account-email">Email address</label><input id="account-email" type="email" required value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" autoComplete="email"/></>}{mode !== "forgot" && <><label htmlFor="account-password">Password</label><input id="account-password" type="password" minLength="8" required value={password} onChange={event => setPassword(event.target.value)} placeholder="At least 8 characters" autoComplete={mode === "signin" ? "current-password" : "new-password"}/></>}<button type="submit">{copy[2]}</button></form>{mode !== "update" && <div className="accountModeLinks">{mode !== "signin" && <button onClick={() => setMode("signin")}>Already have an account? Sign in</button>}{mode === "signin" && <><button onClick={() => setMode("signup")}>Create an account</button><button onClick={() => setMode("forgot")}>Forgot password?</button></>}</div>}</>}{status && <small className="accountStatus">{status}</small>}</section></div>;
}

function SiteMenu({open, onToggle, onAccount, user}) {
  return <div className="siteMenu"><button className="menuButton" onClick={onToggle} aria-expanded={open} aria-label="Open menu">{open ? "×" : "☰"}<small>MENU</small></button>{open && <nav className="menuPanel" aria-label="Main menu"><a href="/about">About us</a><a href="/privacy">Privacy policy</a><a href="/terms">Terms of use</a><a href="/contact">Contact us</a><button onClick={onAccount}>{user ? "My account" : "Sign in"}</button></nav>}</div>;
}
...
function Story({item, index, paletteIndex = index, palette, onRate, onSave, onShare, saved}) {
  ...
  const inspectImage = event => {
    ...
    // RSS publishers commonly provide a clean 800–1200px editorial image.
    // The old 1200x700 gate rejected most of those *after* the server had
    // correctly counted them as visuals, turning an 80% visual edition into
    // colored boxes in the browser. Reject only genuinely unusable thumbnails.
    if (longEdge < 640 || shortEdge < 360 || image.naturalWidth * image.naturalHeight < 300000) setImageRejected(true);
  };
  useLayoutEffect(() => {
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const finishRedirect = async () => {
      const url = new URL(location.href), code = url.searchParams.get("code"), recovering = url.searchParams.get("reset") === "1";
      if (code) await supabase.auth.exchangeCodeForSession(code);
      const {data:{session}} = await supabase.auth.getSession();
      if (active) setUser(session?.user || null);
      if (recovering && active) { setAccountMode("update"); setAccountOpen(true); setAccountStatus("Choose your new password."); }
      if (code || recovering) { url.searchParams.delete("code"); url.searchParams.delete("reset"); url.searchParams.delete("type"); history.replaceState({}, "", `${url.pathname}${url.search}${url.hash}`); }
    };
    finishRedirect();
    const {data:{subscription}} = supabase.auth.onAuthStateChange((event, session) => {
      setUser(session?.user || null);
      if (event === "PASSWORD_RECOVERY") { setAccountMode("update"); setAccountOpen(true); setAccountStatus("Choose your new password."); }
    });
  ...
  useEffect(() => {
    ...
    const loadEdition = async (preserve, force = false) => {
      ...
      try {
        ...
        // Current inventory is delivered inventory. Always exclude it from a
        // new assembly run—even on the same day and even during quiet refresh.
  ...
  const handleAccountSubmit = async event => {
    ...
    if (!supabase) { setAccountStatus("Supabase is not connected to this deployment yet."); return; }
    setAccountStatus("One moment…");
    let result;
    if (accountMode === "signup") result = await supabase.auth.signUp({email:accountEmail,password:accountPassword,options:{emailRedirectTo:location.origin}});
    else if (accountMode === "forgot") result = await supabase.auth.resetPasswordForEmail(accountEmail,{redirectTo:`${location.origin}?reset=1`});
    else if (accountMode === "update") result = await supabase.auth.updateUser({password:accountPassword});
    else result = await supabase.auth.signInWithPassword({email:accountEmail,password:accountPassword});
    if (result.error) { setAccountStatus(result.error.message); return; }
    setAccountPassword("");
    if (accountMode === "signup") setAccountStatus(result.data?.session ? "Account created. You’re signed in." : "Check your email once to confirm your account.");
    else if (accountMode === "forgot") setAccountStatus("Check your email for a password-reset link.");
    else if (accountMode === "update") { setAccountMode("signin"); setAccountStatus("New password saved. You’re signed in."); }
    else setAccountStatus("Signed in. This device will remember you.");
  };
  const signOut = async () => { if (supabase) await supabase.auth.signOut(); setUser(null); setAccountStatus("Signed out on this device."); };
  const openAccount = () => { setMenuOpen(false); setAccountMode("signin"); setAccountPassword(""); setAccountStatus(user ? "Signed in and synced." : ""); setAccountOpen(true); };
  ...
  return <main style={paletteStyle} className={`shell daypart-${daypart} ${identityClass}`} data-editorial-identity={data?.editorialIdentity?.label || "Meanwhile"}>
    ...
    {accountOpen && <AccountPanel user={user} email={accountEmail} setEmail={setAccountEmail} password={accountPassword} setPassword={setAccountPassword} mode={accountMode} setMode={setAccountMode} status={accountStatus} onSubmit={handleAccountSubmit} onSignOut={signOut} onClose={() => setAccountOpen(false)} />}
    <header className="mast"><div className="mastLeft"><SiteMenu open={menuOpen} onToggle={() => setMenuOpen(value => !value)} onAccount={openAccount} user={user}/><button className="refreshButton mastRefresh" onClick={() => refreshEditionRef.current?.()} aria-label="Load a completely fresh edition" title="Load a completely fresh edition"><span>↻</span><small>REFRESH</small></button></div><div className="mastIdentity"><div className="brand brandVignelli" aria-label="Meanwhile">{"Meanwhile".split("").map((letter,index) => <span aria-hidden="true" style={{color:masthead[index]}} key={`${letter}-${index}`}>{letter}</span>)}</div>{editionTitle && <div className="editionName">{editionTitle}</div>}<div className="edition">Rage-free news, discovery & good times</div></div><div className="mastTools"><a className="personalizeButton" href="/make-it-yours">{profile ? "Tune my edition" : "Make it yours"}</a>{profile && <button className="genericButton" onClick={clearProfile}>Generic Edition</button>}<button className="accountButton" onClick={openAccount}>{user ? "My account" : "Sign in"}</button><button className="savedButton" onClick={() => setShowSaved(value => !value)}>Saved <b>{saved.length}</b></button><button className="themeToggle" onClick={() => setTheme(value => value === "dark" ? "light" : "dark")} aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`} title={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}><span>{theme === "dark" ? "☀" : "☾"}</span><small>{theme === "dark" ? "LIGHT" : "DARK"}</small></button></div></header>
    <div className="hello"><h1>{greeting}.</h1><div className="helloAside"><p>{date}</p></div></div>
