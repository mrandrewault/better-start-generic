...
export default function Home() {
  ...
  const uniqueSerendipity = useMemo(() => { const pageSeen = new Set(); [data?.ribbonFavorite, data?.goodNews, ...(data?.favorites || []), ...(data?.important || []), ...wall].filter(item => item && item.format !== "joy").forEach(item => identityKeys(item).forEach(key => pageSeen.add(key))); return claimUnique(data?.serendipity || [], pageSeen); }, [data, wall]);
